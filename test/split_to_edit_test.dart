...
void main() {
  group('SplitToEditDuration', () {
    testWidgets('renders correctly in collapsed state', (WidgetTester tester) async {
    ...
    testWidgets('toggles to editing state when tapping edit icon', (WidgetTester tester) async {
      await tester.pumpWidget(
        const MaterialApp(
          home: Scaffold(
            body: Center(
      ...
      final hourField = tester.widget<TextField>(
        find.ancestor(of: find.text('2'), matching: find.byType(TextField)).first,
      );
      expect(hourField.enabled, isTrue);
    });

    testWidgets('updates values when editing and clicking check', (WidgetTester tester) async {
      int? updatedHours;
      int? updatedMinutes;
      ...
      final hourFinder = find.ancestor(of: find.text('2'), matching: find.byType(TextField)).first;
      await tester.enterText(hourFinder, '5');
      
      // Find the TextField for minutes and enter "45"
      final minFinder = find.ancestor(of: find.text('30'), matching: find.byType(TextField)).first;
