...
void main() {
  group('SignatureDrawPad Widget Tests', () {
    testWidgets('Should render initial state correctly', (WidgetTester tester) async {
      ...
      expect(find.byWidgetPredicate((w) => w is Container && w.constraints?.minHeight == 200), findsOneWidget);
    });



    testWidgets('Should change color when palette is tapped', (WidgetTester tester) async {
      ...
      final redButton = find.byWidgetPredicate(
        (widget) => widget is Container && widget.decoration is BoxDecoration && (widget.decoration as BoxDecoration).color == Colors.red
      );
